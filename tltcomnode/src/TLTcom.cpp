...
#include "tltcomnode/MachineState.h"
#include "tltcomnode/CurrPos.h"
#include "tltcomnode/CurrVel.h"
#include "tltcomnode/ErrorCode.h"
#include "tltcomnode/MoveSpeed.h"
#include "tltcomnode/MovetoPos.h"
#include <sensor_msgs/JointState.h>
...
io_service  io;      
boost::asio::serial_port handle(io);
...
bool send(std::string cmd) {
  ...
  try {
    ...
    std::cerr << "Could not send command '" << cmd << "' towards port " << "." << std::endl;
    return false;
  }
  ...
  if(ROSLdestposition.data.size()==1){    
   ...
   for(uint16_t cnt=0;cnt<2 ;cnt++){
	...
	boost::this_thread::sleep_for(boost::chrono::milliseconds(500));
      ...
      RosListHandl[cnt] =boost::thread(movetoPos,cnt+1,defaultspeed,posdest[cnt]);
    }
   }
}
...
bool MachineStateSRV(tltcomnode::MachineState::Request &req,tltcomnode::MachineState::Response &res){
  res.state =(uint)MachineState;
  return true;
}
bool CurrPosSRV(tltcomnode::CurrPos::Request &req,tltcomnode::CurrPos::Response &res){
  res.sum=currposition[0];
  return true;
}
bool CurrVelSRV(tltcomnode::CurrVel::Request &req,tltcomnode::CurrVel::Response &res){
  res.sum=currVelocidad[0];
  return true;
}
bool ErrorCodeSRV(tltcomnode::ErrorCode::Request &req,tltcomnode::ErrorCode::Response &res){
  res.sum = ErrorCode;
  return true;
}
bool MovetoPos(tltcomnode::MovetoPos::Request &req,tltcomnode::MovetoPos::Response &res){
   boost::float64_t a[2];         
     a[0]=req.Pos1;
     a[1]=req.Pos2;
   for(uint16_t cnt=0;cnt<2 ;cnt++){
...
bool MoveSpeed(tltcomnode::MoveSpeed::Request &req,tltcomnode::MoveSpeed::Response &res){
  boost::float64_t a[2];   
     a[0]=req.vel/2;
     a[1]=req.vel/2;
   for(uint16_t cnt=0;cnt<2 ;cnt++){
...
//MAIN***MAIN***MAIN***MAIN***MAIN***MAIN***MAIN***MAIN***MAIN***MAIN***MAIN***M
    int main(int argc, char* argv[]) {
      ...
      int a;
    if (nh.getParam("port", port)&&nh.getParam("baudrate", baudrate)&&nh.getParam("defaultspeed",a))
    {
	defaultspeed=a;
      ROS_WARN_STREAM("Got param: %s"<<port.c_str()<<" "<<baudrate<<" "<<defaultspeed);
    }
    else
    {
      ROS_ERROR("Failed to get param 'my_param'");
    }
	//baudrate = 38400;                                              /*!< COMM baud rate.                  */
	//port="/dev/ttyUSB0";

      ros::Publisher ROSPPosition = nh.advertise<std_msgs::Float64MultiArray>("/TLT/out/pos", 1);
      ros::Publisher ROSPVelocity = nh.advertise<std_msgs::Float64MultiArray>("/TLT/out/vel", 1);
      ros::Publisher ROSPSensor = nh.advertise<sensor_msgs::JointState>("/TLT/out/SM", 1);
      ros::Publisher joint_pub = nh.advertise<sensor_msgs::JointState>("/TLT/out/joint_states", 1);
      ...
      ros::Subscriber ROSLPosition = nh.subscribe("/TLT/in/pos", 1, roslistenerpos);
      ros::Subscriber ROSLVelocity = nh.subscribe("/TLT/in/vel", 1, roslistenervel);
