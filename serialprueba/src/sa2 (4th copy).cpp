...
bool send(std::string cmd) {
  ...
  ROS_INFO_STREAM( "Enviando: "<<string2hexString(cmd) ); 
  return true;
}
...
void roslistenerpos(const std_msgs::Float64MultiArray& ROSLdestposition){
  ...
  if(ROSLdestposition.data.size()==1){    
   ...
   for(uint16_t cnt=0;cnt<2 ;cnt++){
	...
	std::cout<<(MachineState & mask)<<" "<<cnt<<std::endl;
	...
	boost::this_thread::sleep_for(boost::chrono::milliseconds(500));
      ...
      RosListHandl[cnt] =boost::thread(movetoPos,cnt+1,50,posdest[cnt]);std::cout<<"created Succesfull"<<cnt<<std::endl;
    }
   }
}
